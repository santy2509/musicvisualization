(function() {
  angular.module("pianogramApp", ["Music"])
    .directive("pianogramArcs", pianogramArcs)
  ...
  function pianoCtrl($scope, $interval, $http, MusicService) {
    ...
    // function binKeys
    function binKeys() {
      var denominator = 0;  // calculate denominator to divide by
      var maxCount = 1;  // to calculate maxCount percentage for d3 outer radius scaling
      ctrl.keys.forEach(function(key) {
        if (key.measure <= ctrl.measure.value) denominator++;
      });
      ...
      ctrl.keys.forEach(function(key) {
        ctrl.data.forEach(function(d) {
          if (key.keyId == d.keyId && key.measure <= ctrl.measure.value) {
            d.count += 1;
            d.percentage = d.count / denominator;
          }
          if (maxCount < d.count) maxCount = d.count;
          d.maxCountPercentage = d.count / maxCount;
        });
      });
    }
  ...
  // directive function pianogramArcs
  function pianogramArcs() {
    return {
      restrict: "E",
      scope: {
        data: "=",
